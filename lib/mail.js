...
/**
 * mail.PROFILES -> Object
 *
 * Contains multiple SMTP configuration profiles. The configuration set in 
 * mail.SMTP is the default profile, which can be accessed as 
 * mail.PROFILES.default. Setting options in mail.PROFILES.default is 
 * equivalent of setting mail.SMTP directly.
 *
 * Each additional profile is stored in a named key in mail.PROFILES. For 
 * example:
 * 
 *     mail.PROFILES.mymail = {
 *         host: 'smtp.mymail.com',
 *         user: 'myuser',
 *         pass: 'mypass
 *     };
 *
 * This can then be used by send_mail:
 *
 *     mail.send_mail(params, 'mymail', callback);
 *
 * or when creating a new EmailMessage object:
 *
 *     var em = EmailMessage(params, 'mymail');
 *
 * Assigning an Object literal to mail.PROFILES destroys the link between a 
 * profile called 'default' and mail.SMTP object. If you do not specify the 
 * 'default' profile in the object literal, or mail.SMTP object separately, 
 * there will no longer be a fallback configuration. If you have to use an
 * Object literal for setting mail.PROFILES, make sure there is a 'default' 
 * profile.
 *
 * See documentation of mail.SMTP object for configuration options for 
 * individual profiles.
 **/
exports.PROFILES = (function(exports) {
    function Profiles() {
        this.__defineGetter__('default', function() {
          return exports.SMTP;
        });
        this.__defineSetter__('default', function(conf) {
          exports.SMTP = conf;
        });
    }
    return new Profiles();
...
/**
 * mail.send_mail(params, profile, callback) -> undefined
 * - params (Object): e-mail data
 * - profile (String): optional configuration profile to override the default
 * - callback (Function): will be run after completion
 * 
 * Shortcut function to send e-mails. See EmailMessage for param structure
 **/
exports.send_mail = function(params, profile, callback){
    if (typeof callback === 'undefined') {
      callback = profile;
      profile = null;
    }
    var em = new EmailMessage(params, profile);
    em.send(callback);
    return em;
};
...
/**
 * new mail.EmailMessage(params, profile)
 * - params (Object): message data (can be altered afterwards)
 * - profile (String): optional configuration profile to override default
 * 
